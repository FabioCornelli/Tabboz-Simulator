...
//			    break;
		    ...
		    else if (!strcmp(argv[0],"ver")) {		/* VER ------------------------------------ */
			sprintf(tmp,"Tabboz Simulator %s",VERSION);                writeME(tmp);
			sprintf(tmp,"  %s",__DATE__);                              writeME(tmp);
			sprintf(tmp,"  Copyright(C) 1997,1998");                   writeME(tmp);
			sprintf(tmp,"  by Obscured Truckware");                    writeME(tmp);
			sprintf(tmp,"  Created by:");                              writeME(tmp);
			sprintf(tmp,"     %s",Andrea);                             writeME(tmp);
			sprintf(tmp,"     %s",Caccia);                             writeME(tmp);
			sprintf(tmp,"     %s",Daniele);                            writeME(tmp);

			break;
		    }
		    else if (!strcmp(argv[0],"status")) {	/* STATUS --------------------------------- */
			sprintf(tmp, "Nome        = %s %s",Nome,Cognome);        writeME(tmp);
			sprintf(tmp, "Soldi       = %s",MostraSoldi(Soldi));    writeME(tmp);
			sprintf(tmp, "Figosita'   = %d/100",Fama);          writeME(tmp);
			sprintf(tmp, "Reputazione = %d/100",Reputazione); writeME(tmp);
			sprintf(tmp, "Prof.Scol.  = %d/100",Studio);       writeME(tmp);

			if ( Rapporti != 0 ) {
				sprintf(tmp, "Tipa        = %s",Nometipa);           writeME(tmp);
				sprintf(tmp, "  Rapporti  = %d/100",Rapporti); writeME(tmp);
			}
			if (ScooterData.stato != -1) {
				sprintf(tmp, "Scooter     = %s",ScooterData.nome);          writeME(tmp);
				sprintf(tmp, "  Stato     = %d/100",ScooterData.stato); 	writeME(tmp);
			}
			sprintf(tmp, "%s %d %s",InfoSettimana[x_giornoset-1].nome,x_giorno,InfoMese[x_mese-1].nome); writeME(tmp);
			break;
		    }
		    else if (!strcmp(argv[0],"date")) {		/* DATE ----------------------------------- */
		    ...
		    else if (!strcmp(argv[0],"about")) {	/* ABOUT ---------------------------------- */
			/* Display about box. */
		    ...
		    else if (!strcmp(argv[0],"logo")) {		/* LOGO ----------------------------------- */
		    ...
		    else if (!strcmp(argv[0],"config")) {	/* CONFIG --------------------------------- */
