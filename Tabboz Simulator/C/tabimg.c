...
int static NEAR PASCAL WMDestroy( HWND hwnd )
{
	...
	if( hbmp_testa ) 	    { DeleteObject(hbmp_testa);      RemoveProp(hwnd,"hbmp_testa");    }
	if( hbmp_giubbotto )  { DeleteObject(hbmp_giubbotto);  RemoveProp(hwnd,"hbmp_giubbotto");}
	if( hbmp_pantaloni )  { DeleteObject(hbmp_pantaloni);  RemoveProp(hwnd,"hbmp_pantaloni");}
	if( hbmp_scarpe ) 	 { DeleteObject(hbmp_scarpe);     RemoveProp(hwnd,"hbmp_scarpe");   }
	if( hbmp_sfondo ) 	 { DeleteObject(hbmp_sfondo);     RemoveProp(hwnd,"hbmp_sfondo");   }

	if( hmask_testa ) 	 { DeleteObject(hmask_testa); 	 RemoveProp(hwnd,"hmask_testa"); 	 }
	if( hmask_giubbotto ) { DeleteObject(hmask_giubbotto); RemoveProp(hwnd,"hmask_giubbotto");}
	if( hmask_pantaloni ) { DeleteObject(hmask_pantaloni); RemoveProp(hwnd,"hmask_pantaloni");}
	if( hmask_scarpe ) 	 { DeleteObject(hmask_scarpe); 	 RemoveProp(hwnd,"hmask_scarpe"); 	 }
	if( hmask_sfondo ) 	 { DeleteObject(hmask_sfondo); 	 RemoveProp(hwnd,"hmask_sfondo"); 	 }
...
int static NEAR PASCAL WMPaint( HWND hwnd )
{
	...
	if (TabbozRedraw) {	// Ricarica le immagini...
		WMDestroy(hwnd);
		WMCreate(hwnd,NULL);
	}
	...
	// Mantenere l' ordine !!!
	if(hbmp_sfondo)    DrawTransparentBitmap(hdc,hbmp_sfondo,   hmask_sfondo,     0 				 ,  0 			  ,IMG_SIZEX,IMG_SIZEY,0,0);

	if (sesso == 'M') {
		if(hbmp_pantaloni) DrawTransparentBitmap(hdc,hbmp_pantaloni,hmask_pantaloni, 18 + IMG_X_INC, 93 + IMG_Y_INC,IMG_SIZEX,IMG_SIZEY,0,0);
	} else {
		if(hbmp_pantaloni) DrawTransparentBitmap(hdc,hbmp_pantaloni,hmask_pantaloni, 23 + IMG_X_INC, 120 + IMG_Y_INC,IMG_SIZEX,IMG_SIZEY,0,0);
	}


	if(hbmp_scarpe)    DrawTransparentBitmap(hdc,hbmp_scarpe,   hmask_scarpe,   -16 + IMG_X_INC,205 + IMG_Y_INC,IMG_SIZEX,IMG_SIZEY,0,0);

	if (sesso == 'M') {
		if(hbmp_giubbotto) DrawTransparentBitmap(hdc,hbmp_giubbotto,hmask_giubbotto,-30 + IMG_X_INC, 18 + IMG_Y_INC,IMG_SIZEX,IMG_SIZEY,0,0);
		if(hbmp_testa)     DrawTransparentBitmap(hdc,hbmp_testa,    hmask_testa,     32 + IMG_X_INC,  3 + IMG_Y_INC,IMG_SIZEX,IMG_SIZEY,0,0);
	} else {
		if(hbmp_giubbotto) DrawTransparentBitmap(hdc,hbmp_giubbotto,hmask_giubbotto,-27 + IMG_X_INC, 39 + IMG_Y_INC,IMG_SIZEX,IMG_SIZEY,0,0);
		if(hbmp_testa)     DrawTransparentBitmap(hdc,hbmp_testa,    hmask_testa,     30 + IMG_X_INC,  1 + IMG_Y_INC,IMG_SIZEX,IMG_SIZEY,0,0);
	}
